 ...
 * enable them
 */
    /* default_ldt是一个描述符变量，这句将该变量初始化为调用门
       的一个描述符，处理代码为lcall7 */
	...
	if ((unsigned long)&end >= (1024*1024)) {
		/* 用来区分是比较大的zImage还是比较小的image，
		   因为只有较大的zImage才会被解压缩到1M以后的内存中*/
		/*
		 * memory_start指的是内核代码和数据之后的自由内存的开始
		 * low_memory_start指的是1M之前的自由内存区域
		 */
	...
	/* 函数执行完后，memory_start变大，因为有一部分内存用来分配页表用了 */
	memory_start = paging_init(memory_start,memory_end);
	if (strncmp((char*)0x0FFFD9, "EISA", 4) == 0)
		EISA_bus = 1;

	/* 初始化中断向量表, 然后初始化的trap错误 */
	trap_init();
