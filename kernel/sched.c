...
void sched_init(void)
{
	...
	/* init_task是内核创建的第一个进程结构体 */
	/* gdt结构：在几个公用的cs、ds等选择子之后，是连续的tss,ldt序列对 */
	set_tss_desc(gdt+FIRST_TSS_ENTRY,&init_task.tss);
	/* default_ldt只有一个描述符，
	   在start_kernel最开始的时候被初始化成了一个调用门，
	   指向函数lcall7 */
    ...
    /* gdt用于tss和ldt的描述符清空，task是内核的进程结构体数组。
       在新型的Linux内核中，task全部用链表做了实现，*/
    /* 事实上，数组的大小指定了内核所允许的最多的进程数 */
    ...
    /* NT(nested task)位，eflags的14bit，如果因为call,中断、异常而切换到其他任务时，该位置1 */
